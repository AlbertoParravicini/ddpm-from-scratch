...
class UpDownBlock(nn.Module):
    ...
    def forward(
        ...
        return x + expand_to_dims(self.timestep_embedding(t), x)
...
class UNet(nn.Module):
    def __init__(
        ...
        Time-conditioning is applied inside each ResNet block.
        ...
        for i in range(len(self._channels) - 1):
            self.downsample_layers += [UpDownBlock(self._channels[i], self._channels[i + 1], down=True)]
